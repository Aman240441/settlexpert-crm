...
import { Eye, EyeOff, Shield, FileImage, AlertCircle, CheckCircle2, Download, X } from 'lucide-react';
...
export const AadhaarSection: React.FC<AadhaarSectionProps> = ({
  ...
  const [fullAadhaar, setFullAadhaar] = useState<string | null>(null);
    ...
    <div className="space-y-4">
      ...
      {isAdmin && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <AadhaarDocCard label="Aadhaar Front" hasDoc={!!hasFront} docData={frontDocData} loading={loadingFront} onView={() => handleViewDoc('front')} />
          <AadhaarDocCard label="Aadhaar Back" hasDoc={!!hasBack} docData={backDocData} loading={loadingBack} onView={() => handleViewDoc('back')} />
        </div>
      )}
...
const AadhaarDocCard: React.FC<AadhaarDocCardProps> = ({ label, hasDoc, docData, loading, onView }) => (
  <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
    ...
    <div className="p-5 flex items-center justify-center min-h-[120px]">
      {!hasDoc ? (
        ...
        <img
          ...
          alt={label}
          className="max-h-[100px] object-contain rounded-lg cursor-pointer hover:opacity-90 transition-opacity shadow-sm"
          onClick={onView}
        />
      ) : (
        <div className="text-center">
          <FileImage className="h-8 w-8 text-slate-300 mx-auto mb-2" />
          <p className="text-xs text-slate-500 mb-3">Document available</p>
          <button onClick={onView} disabled={loading} className="px-3 py-1.5 bg-slate-800 text-white text-[11px] font-bold rounded-lg hover:bg-slate-700 transition-colors flex items-center gap-1.5 mx-auto">
            <Eye className="h-3 w-3" />{loading ? 'Loading...' : 'View Document'}
          </button>
        </div>
      )}
    </div>
    {hasDoc && docData && (
      <div className="px-4 pb-3 flex gap-2">
        <button onClick={onView} className="flex-1 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-bold rounded-lg flex items-center justify-center gap-1.5 transition-colors">
